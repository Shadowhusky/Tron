...
- **Pane maximize**: `Tab.maximizedSessionId` (persists via the `{ ...tab }` layout spread). `toggleMaximizePane(sessionId?)` in LayoutContext; `SplitPane` collapses non-containing branches to `flex: 0` (panes stay MOUNTED — visibility via zero size + overflow-hidden, animated with a 240ms flex-grow transition) and hides dividers. The toggle dispatches `tron:splitDragStart`/`End` so terminals do one clean `fit()` after the transition; `performResize` skips containers < ~80px in either axis so collapsed panes never shrink the PTY. Cleared whenever a pane is added (splits, editor/browser split) or the maximized pane closes; sanitized after SSH id remaps at hydration. Entry points: hover button top-right of the terminal area + agent-mode header button + context menu + palette + `maximizePane` hotkey (⌘⇧M), all showing the hotkey via `title`/`formatHotkey`. `SplitPane` receives the OWNING tab's `maximizedSessionId` as a prop from App's per-tab render (never the active tab's context value) — hidden tabs must keep their collapsed layout or switching back re-animates the collapse (flicker).
...
- **Shell-exit recovery**: When a PTY dies mid-session, main prints `[Process exited with code N]` into the pane and sends `terminal.exit`; LayoutContext marks `TerminalSession.exited` (SSH skipped — own reconnect path) and TerminalPane shows a "Restart Shell" overlay. `restartShell(sessionId)` respawns via `createPTY(cwd, sessionId)` under the SAME id, so the mounted Terminal keeps its buffer and IPC wiring. Never leave a dead pane silent — `terminal.write` to a missing session is a silent no-op.
...
- **Prompt queue + steering** (`TerminalPane.tsx` + `aiService`): prompts sent while the agent runs are QUEUED (`inputQueue`, per-pane; items carry `id` + optional `images`). ⌥⏎ (or a queued row's ↗ button) STEERS instead: `aiService.postSteeringMessage(sessionId, text)` → `runAgent` drains it at the top of each loop iteration, pushes a `[USER UPDATE]` user message into history and emits a `steered` step (rendered in AgentOverlay; stale messages cleared at run start; leftovers posted too late demote back to the queue front). The queue UI (above SmartInput) is a Warp-style list: drag handle reorder (`Reorder.Group`, `dragListener=false` + handle), click-to-edit inline (Enter save / Esc cancel / empty deletes), per-row steer + delete, Esc in an EMPTY SmartInput pops the newest queued item back for editing. Drains never drop silently: an item is removed only when its run starts — `handleAgentRun` returns `accepted: boolean` (false = empty / concurrent run raced in / throttled), rejected drains re-queue at the FRONT and retry via `drainNonce`, a 2s watchdog retries while idle with a non-empty queue, and queue-sourced runs bypass the 500ms cooldown (it used to silently eat dequeued prompts). Manual stop PAUSES the queue visibly (amber "paused — resume" chip); any new run resumes it. While running, SmartInput also shows a blue ⏎ QUEUE button beside stop (⌥-click steers) so queueing isn't Enter-only; the drain holds while the front item is being inline-edited.
...
- **Tab wheel**: game-style radial switcher (`src/components/ui/TabWheel.tsx`). HOLD the `tabWheel` hotkey (default ⌃Tab) → wheel appears; mouse ANGLE from screen center picks the sector (pure direction, any distance, 24px deadzone); releasing any combo key commits, Esc/backdrop cancels, clicking a sector commits immediately. App.tsx opens it via a capture-phase keydown (xterm never sees the key); keyup/commit handling lives inside TabWheel. Geometry + combo parsing in `src/utils/tabWheel.ts` (pure, unit-tested): sector 0 centered at 12 o'clock, clockwise.
...
- `web_search` — searches the web via `web.search` IPC. Backend chain: Brave Search **API** (only when `BRAVE_SEARCH_API_KEY` is set) → DuckDuckGo lite → DuckDuckGo html → Brave HTML scrape. Parsers are pure + unit-tested in `electron/ipc/webSearchParse.ts` (mirrored in `server/index.ts`). **Keyless scraping is best-effort** — every free endpoint rate-limits or captcha-walls (verified live: Brave 429, DDG 202 "anomaly", Mojeek captcha, Startpage consent wall), so the chain distinguishes `failure: "blocked"` (every backend refused) from `failure: "empty"` (a working backend returned nothing). That distinction matters: `describeSearchFailure()` (`src/utils/searchFailure.ts`) tells the agent to STOP searching and `web_fetch` a known URL when blocked, instead of the old "reformulate your keywords" advice which made it reword queries against a dead service for 4 turns (log 39172cb246).
...
### Safety Mechanisms
- **Loop detection**: Blocks after 2 consecutive identical calls (5 for `send_text`) or A→B→A→B alternation. `actionKey` includes `tool`, `path`, `command`, `text`, `query`, `url` — so `web_search` with different queries won't trigger. Escalates after 3 breaks. `final_answer`/`ask_question` are EXEMPT (loop-blocking a final_answer while advising "use final_answer" is a deadlock — observed log fd5977963a).
- **Every bounce path is bounded** (all counters reset when a non-terminal action is accepted for dispatch; verified end-to-end in `src/__tests__/agentRunLoop.test.ts` with a scripted fake model): API/model errors retry the SAME request silently with nothing pushed to history (an `(API error)` placeholder turn taught weak models to echo it) and fail after 3; `final_answer`/`ask_question` rejected 3× → force-resolve (fail if no work was done, else surface the model's answer / let the question through); a loop-BLOCKED action re-emitted 3× → `escalateStuck`; variations of a cap-exhausted shape after STOP 3× → `escalateStuck`.
- **Circuit breaker**: After 3 consecutive guard blocks, resets scaffold guards (`usedScaffold`) so the next command can run — it must NOT clear `executedCommands` (that's the "work was done" evidence for the final_answer guards). 3 trips without a successful action in between → `escalateStuck`.
...
- **Planning is opt-in, not forced** (log 39172cb246: "whats todays weather" produced a 4-item todo list, 4 plan emissions and ~19 model round-trips in 33s). Following Claude Code's TodoWrite rules and Codex's plan-tool guidance, `agent.md` + the system prompt now tell the model to skip `todo_write` for informational/conversational requests, 1–2 step tasks, and the easiest ~25% of work, plus a SIMPLE REQUESTS rule (`date` for the time, `wttr.in/?format=j1` for weather — it IP-geolocates) with a ~2 tool-call target. The forcing nudges are gone: the old "[plan check] … your NEXT response MUST be todo_write" at 5 substantive steps is deleted (5 tool calls is normal work), the 12-step one is now an advisory "[check-in]" offering plan/ask/answer/carry-on, and the staleness reminder fires at 6 steps (was 4) with Claude-Code-style soft wording. A 1-item plan is no longer told to "split into 3+ sub-steps" (which invited padding) — it's told to just do the work.
- **Malformed tool calls never crash the run** (log ddb57d1bbd): weak models emit calls with the arguments missing (`{"tool":"execute_command"}` with no `command`), and `action.command.split(...)` threw a raw TypeError that escaped `runAgent` and ended the run after one good step. A **required-argument gate** now runs before dispatch, deriving each tool's required fields from `AGENT_TOOLS` via `missingToolArgs()` (one source of truth), and hands the model a `<tool_use_error>` naming exactly what it omitted. Behind that, a **per-step error boundary** wraps the loop body: an unexpected throw is surfaced as a `failed` step, reported to the model, resets `terminalBusy` (set before awaits, so a throw mid-await would otherwise wedge the agent into "terminal permanently busy"), and is bounded by `MAX_UNEXPECTED_ERRORS = 3` → `escalateStuck`. The body is deliberately not re-indented into the `try` so the safety net stays a readable diff.
- **Conversation context scales with the model** (`src/utils/modelContext.ts`, `src/utils/agentContext.ts`): `AIConfig.contextWindow` is one manual number in CHARS defaulting to 16k, so a 200k-token cloud model was clipped as hard as a 7B local one — an earlier task reached the agent as "…allocation of current machine'…" and a follow-up "continue" had nothing to continue from. `contextCharsFor(model, configured)` infers a real window (explicit `-32k` in the model id wins, then a family table, then the 16k default; a user-set value always wins), `conversationBudgetChars()` gives the conversation 25% capped at 60k, and `buildOlderConversation()` fills that budget newest-first — whole turns while they fit, then one role-clipped turn. `summarizeOlderInteraction()` clips a **user instruction at 300 chars and an agent turn at 100** (it used to be a flat 80 for both, which mangled the one thing the agent can't reconstruct). ContextBar's gauge and the 90% auto-summarize trigger use the same derived window.
- **`read_history` tool**: recalls earlier turns verbatim, filtered by an optional `query`, capped at 8KB. `useAgentRunner` passes the full untruncated thread to `runAgent` via `options.conversation`. The prompt tells the model to call it FIRST when a short follow-up ("continue", "again", "more") is ambiguous, instead of guessing.
...
- **AgentOverlay panel UI**: the pinned plan card is COLLAPSIBLE (header click; collapsed = header + animated progress bar + current in-progress item; spring height animation, `useReducedMotion` respected). ThinkingBlock has a collapse affordance at the BOTTOM as well as the header. Scroll-flicker fixes: virtualizer uses STABLE `getItemKey`s (`s${globalIdx}`/`g${runIdx}`) so the measurement cache survives list growth, overscan 8 so re-measurement happens off-screen, and the modern theme's `backdrop-blur` lives on an isolated `absolute inset-0 -z-10` layer (blur on the scroller's ancestor recomposited every scroll frame → app-wide flicker). New steps animate in exactly once via `lastAnimatedIdxRef` + `.step-enter` CSS (index.css, `prefers-reduced-motion` disabled) — never replayed on virtualizer remounts.
...
- **Native tool calls (learned per model)**: tool-trained models on LM Studio (Harmony/gpt-oss-style, e.g. muse-glimmer) answer the JSON protocol with a NATIVE tool call; without declared `tools` the server buffers it (nothing streams for ~50s after the reasoning) and rejects it with 500 "does not match the expected peg-native format" — the reply is lost. `src/services/ai/toolSchemas.ts` holds OpenAI function declarations for every agent tool (`AGENT_TOOLS`, same parameter names as the JSON protocol; `TOOL_NAMES` derives from it) and `ToolCallAssembler`, which folds streamed `tool_calls` deltas back into `{"tool":…}` JSON (progress streamed to the UI, well-formed text at the end) so parser/guards/history are untouched. Tools are NOT declared by default — the rendered schemas cost ~1–2k prompt tokens and overflowed qwen3.8 @ 8k context. Instead the first mismatch error flips `useNativeTools` for the run, persists the model in `NATIVE_TOOL_MODELS` (`markModelNeedsNativeTools`/`isNativeToolsModel`), and retries the same request with tools (not counted as an API error). Persistent failure with tools → normal 3-strike error with a hint. Only the OpenAI-compatible path (`streamOpenAIChat` `tools` param → `parseOpenAIStream`) — Ollama/Anthropic untouched.
- **Bare tool arguments (shape inference)**: the same tool-trained models also leak native-format ARGUMENTS into plain text — `{"todos":[...]}` with no `{"tool":…}` wrapper, or a lone todo item `{"content":…,"status":"in_progress"}` (qwen3.8, live 2026-08-15). Unhandled, the former burns 5 silent parse retries per step (30–80s each on local models) and the latter is coerced into a bogus `final_answer` the completion guards reject forever. `inferToolFromShape` in `src/utils/agentLoop.ts` (pure, unit-tested) maps unambiguous argument-key signatures to tools (`todos`→todo_write, `path`+`content`→write_file, `path`+`search`+`replace`→edit_file, `url`→web_fetch, `query`→web_search, todo-item→1-item todo_write, etc.), called from `normalizeToolKey` so every parse path (direct, markdown, balanced-scan, thinking fallback) benefits. Ambiguous shapes (bare `path`, bare `content`) return null — existing coercion/retry handles them. `parseXmlToolCall` handles the third leak shape: Hermes-style XML tool calls as plain text (`<tool_call><function=search_dir><parameter=path>…`, malformed quoting tolerated, missing closers tolerated) — unparsed these became a bogus "done" final_answer echoing raw XML (log d3f522fd6d). `looksLikeToolCallText` additionally guards BOTH plain-text-coercion and the retries-exhausted fallback: text that looks like an attempted tool call (XML or JSON) is never surfaced as a final answer.
- **Reasoning display follows the model, not the toggle**: `thinkingEnabled` only governs *requesting* thinking (and LM Studio can't switch it off). Any reasoning the model actually streams (`reasoning_content` or `<think>` tags) is emitted as `streaming_thinking` via the single `emitThinking` helper in `runAgent`, and first sight of `reasoning_content` calls `markModelAsThinking` so the toggle appears for that model. Hiding it made a Harmony-format model (muse-glimmer: 60s+ of analysis before any content) look hung (log 52e887da4e). If an attempt dies mid-stream after streaming reasoning, the API-error path emits `retract_thought` then a visible `failed` "retrying (n/3)" line; context-overflow errors fail immediately, and LM Studio's "peg-native format" parse error triggers the native-tools switch below.
...
- **AI CLI auto-resume after restart** (`src/utils/agentRecovery.ts` + `electron/ipc/agentSessions.ts`, unit-tested): PTYs die on app quit, but Claude Code / Codex persist their own conversations. At restore time (fresh PTY + persisted history, NOT page-refresh reconnect or saved-tab load), `detectResumableAgent(history)` decides the CLI was live at quit — gates: tail not an idle shell prompt (incl. bare starship `❯`), brand markers in the recent region (`detectAgentBrand` in `externalAgentStatus.ts`), live input-frame/footer evidence in the bottom ~25 lines (`hasLiveAgentFrame`). The EXACT session id is then resolved via `agent.findResumeSession` (Electron main): `extractResumeFragments()` pulls distinctive transcript lines from the pane tail; the main process fingerprints them (JSON-escaped substring match) against the CLI's own session store — `~/.claude/projects/<munged-cwd>/<uuid>.jsonl` (claude) / `~/.codex/sessions/YYYY/MM/DD/rollout-*.jsonl` filtered by `session_meta.cwd` (codex) — and returns an id only on a confident unique match (winner needs ≥3 hits and 2× separation from runner-up); ambiguity → null → no resume ("most recent" is never guessed — wrong with multiple panes in one cwd). Terminal.tsx polls `readScreenBuffer` every 500ms (15s cap) and, at a clean idle prompt (or 2-tick-stable bare `❯`), types `claude --resume <id>` / `codex resume <id>` (uuid-shape validated). A module-level once-per-session Set guards remounts (never type into a running CLI); any real user keystroke cancels. Electron-only (web/demo invoke fails → silently skipped). Aider/Cursor are detected but have no resume command.
