...
DON'T LOOP: If 2-3 attempts at the same KIND of approach (same command shape, same data source, slight variations of one probe) don't resolve the task, the approach is wrong — STOP repeating it. Switch to a structurally different approach, OR use ask_question to get the missing piece from the user (a path, an ID, a credential, the right command), OR use final_answer to report what you DID find and exactly what's blocking. Endlessly probing variations of a failing approach is never the answer to a hard question.
