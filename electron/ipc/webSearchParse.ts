 ...
 */
export function isBlockedResponse(status: number, html: string): boolean {
  if (status === 429 || status === 403 || status === 202) return true;
  const head = html.slice(0, 4000);
  return (
    /<title>\s*captcha/i.test(head) ||
...
export function parseBrave(html: string): SearchResult[] {
  const out: SearchResult[] = [];
  const blocks = html.split(/class="snippet[\s"]/).slice(1, 12);
  for (const b of blocks) {
    const urlMatch = b.match(/href="(https?:\/\/[^"]+)"/);
    const titleMatch =
      b.match(/class="[^"]*snippet-title[^"]*"[^>]*>([\s\S]*?)<\/div>/) ||
      b.match(/class="title[^"]*"[^>]*>([\s\S]*?)<\/div>/);
    const descMatch =
      b.match(/class="snippet-description[^"]*"[^>]*>([\s\S]*?)<\/p>/) ||
...
export function parseBraveApi(json: unknown): SearchResult[] {
