...
function scoreFile(filePath: string, fragments: string[]): number {
  let content: string;
  try { content = readSlice(filePath, TAIL_BYTES, true); } catch { return 0; }
  let hits = 0;
  for (const frag of fragments) {
    const escaped = JSON.stringify(frag).slice(1, -1);
    if (content.includes(escaped)) hits++;
  }
  return hits;
}
...
function pickConfident(scored: { id: string; hits: number }[], fragmentCount: number): string | null {
  if (fragmentCount < 2) return null;
  const sorted = [...scored].sort((a, b) => b.hits - a.hits);
  const best = sorted[0];
  if (!best) return null;
  const second = sorted[1]?.hits ?? 0;
  if (best.hits < Math.min(3, fragmentCount)) return null;
  if (best.hits < second * 2 || best.hits - second < 2) return null;
  return best.id;
}

function findClaudeSession(cwd: string, fragments: string[]): string | null {
  const dirName = cwd.replace(/[^a-zA-Z0-9]/g, "-");
  const dir = path.join(os.homedir(), ".claude", "projects", dirName);
  let candidates: { id: string; file: string; mtime: number }[];
  try {
  ...
  } catch { return null; } // no store / never ran claude here
...
function readRolloutMeta(filePath: string): { id: string; cwd: string } | null {
  let head: string;
  try { head = readSlice(filePath, 65536, false); } catch { return null; }
  ...
  if (!m) return null;
  try { return { id: m[1], cwd: JSON.parse(`"${m[2]}"`) }; } catch { return null; }
}

function findCodexSession(cwd: string, fragments: string[]): string | null {
  const root = path.join(os.homedir(), ".codex", "sessions");
  const rollouts: { file: string; mtime: number }[] = [];
  try {
    const dateDirs = (dir: string, re: RegExp) =>
      fs.readdirSync(dir).filter((d) => re.test(d)).sort().reverse();
    outer:
    for (const y of dateDirs(root, /^\d{4}$/)) {
      for (const m of dateDirs(path.join(root, y), /^\d{2}$/)) {
        for (const d of dateDirs(path.join(root, y, m), /^\d{2}$/)) {
          ...
          if (rollouts.length >= MAX_ROLLOUT_SCAN) break outer;
        }
      }
    }
  } catch { return null; }
  ...
  for (const r of rollouts.sort((a, b) => b.mtime - a.mtime)) {
    ...
    if (meta && meta.cwd === cwd) candidates.push({ id: meta.id, file: r.file });
  }
  const scored = candidates.map((c) => ({ id: c.id, hits: scoreFile(c.file, fragments) }));
  return pickConfident(scored, fragments.length);
}
...
export function registerAgentSessionHandlers() {
  ipcMain.handle("agent.findResumeSession", (_event, req: { brand?: string; cwd?: string; fragments?: unknown }) => {
    const { brand, cwd } = req || {};
    const fragments = Array.isArray(req?.fragments)
      ? req.fragments.filter((f): f is string => typeof f === "string" && f.length >= 8).slice(0, 16)
      : [];
    if (!cwd || fragments.length < 2) return null;
    try {
      const sessionId =
        brand === "claude" ? findClaudeSession(cwd, fragments)
