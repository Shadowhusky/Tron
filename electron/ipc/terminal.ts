...
// =============================================================================
function _normalizeForEdit(s: string): string {
  return s
    .replace(/\r\n/g, "\n")
    .replace(/[“”‟″❝❞＂]/g, '"')
    .replace(/[‘’‚‛′❛❜＇]/g, "'")
...
function _indexOfAll(hay: string, needle: string): number[] {
  if (!needle) return [];
  const out: number[] = [];
  let i = 0;
...
function _restyleEdit(replace: string, originalSpan: string): string {
...
function applyTolerantEdit(content: string, search: string, replace: string): { content: string; replacements: number; mode: "exact" | "normalized" } | null {
...
interface DisplayBuffer { data: string; timer: ReturnType<typeof setTimeout> | null; send: (cleaned: string) => void; holdbacks: number }
...
export function registerTerminalHandlers(
  ...
  ipcMain.handle("terminal.getShellHistory", () => getShellHistory());
