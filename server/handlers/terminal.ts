...
function applyTolerantEdit(content: string, search: string, replace: string): { content: string; replacements: number; mode: "exact" | "normalized" } | null {
  ...
  if (exact.length > 0) {
    let updated = "";
    let cursor = 0;
    for (const idx of exact) { updated += content.slice(cursor, idx) + replace; cursor = idx + search.length; }
  ...
  while (oi < content.length && ni < nContent.length) {
    if (content[oi] === "\r" && content[oi + 1] === "\n") { map[ni] = oi + 1; oi += 2; ni += 1; continue; }
    map[ni] = oi; oi += 1; ni += 1;
  }
  let updated = "";
  let cursor = 0;
...
export function updateClientPushEvent(clientId: string, pushEvent: EventPusher) {
  ...
  for (const [sessionId, owner] of sessionOwners.entries()) {
    if (owner === clientId) {
      ...
      if (session) {
        ...
        if (idx !== -1) sessionCreationOrder.splice(idx, 1);
      }
...
export function createSession(
  ...
  while (sessions.size >= MAX_SESSIONS && sessionCreationOrder.length > 0) {
    ...
    if (oldPty) {
      ...
      try { oldPty.kill(); } catch { /* already dead */ }
...
export function editFile(filePath: string, search: string, replace: string): { success: boolean; replacements?: number; mode?: string; error?: string } {
  try {
    if (!fs.existsSync(filePath)) {
      return { success: false, error: `File not found: ${filePath}` };
    }
