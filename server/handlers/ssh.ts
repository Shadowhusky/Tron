...
export interface SSHProfile {
  ...
  privateKeyPath?: string;
  saveCredentials?: boolean;
  savedPassword?: string;    // only stored if saveCredentials is true
...
// Matches the subset of node-pty.IPty used by terminal.ts
...
export class SSHSession implements PtyLike {
  pid: number;
  ...
  constructor() {
    ...
    this.config = {} as SSHConnectionConfig;
  }
...
export function readProfiles(): SSHProfile[] {
  try {
...
export async function createSSHSession(
  config: SSHConnectionConfig & { cols?: number; rows?: number; sessionId?: string },
  clientId: string,
  pushEvent: EventPusher,
  sessionMap: Map<string, PtyLike>,
...
export async function testConnection(
  config: Omit<SSHConnectionConfig, "id" | "name">,
...
export function cleanupClientSSHSessions(
  clientId: string,
  ownerMap: Map<string, string>,
) {
